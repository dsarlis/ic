...
use ic_types_cycles::{
    CanisterCyclesCostSchedule, CompoundCycles, Cycles, CyclesUseCase, Instructions, NonConsumed,
...
impl SystemStateModifications {
    ...
    pub fn apply_balance_changes(
        ...
        for (_use_case, _amount) in self.consumed_cycles_by_use_case.iter() {
            // state.remove_cycles(CompoundCycles::new(*amount, *use_case, cost_schedule));
        }
    ...
    fn add_consumed_cycles(
        &mut self,
        consumed_cycles: impl Iterator<Item = (CyclesUseCase, Cycles)>,
    ) {
        for (use_case, amount) in consumed_cycles {
            *self
...
impl SandboxSafeSystemState {
    ...
    pub(super) fn cycles_burn128(
        ...
        self.update_balance_change_consuming(
            new_balance,
            [(CyclesUseCase::BurnedCycles, burned_cycles)].into_iter(),
        );
        burned_cycles
    }
    ...
    pub fn push_output_request(
        ...
        *used_slots += 1;
        let consumed_cycles = vec![
            (
                CyclesUseCase::Instructions,
                instructions_consumed_cycles.real(),
            ),
            (
                CyclesUseCase::RequestAndResponseTransmission,
                request_and_response_trasmission_consumed_cycles.real(),
            ),
        ]
        .into_iter()
        .map(|(use_case, amount)| (use_case, amount));
