...
impl Registry {
        ...
        let new_subnet_admins = match payload.operation_type {
            Some(OperationType::Add(principal_ids)) => {
                ...
                for principal_id in principal_ids {
                    ...
                    if current_subnet_admins.contains(&principal_id_proto) {
                        println!(
                            "{}do_update_subnet_admins: Error: Super-user {:?} is already in current list, so it cannot be added.",
            ...
            Some(OperationType::Remove(principal_ids)) => {
                ...
                for principal_id in principal_ids {
                    ...
                    if !current_subnet_admins.contains(&principal_id_proto) {
                        println!(
                            "{}do_update_subnet_admins: Error: Super-user {:?} is not in current list, so it cannot be removed.",
                            LOG_PREFIX, principal_id
                        );
                        return UpdateSuperUsersResult::Err(Some(
    ...
    fn has_duplicates(principal_ids: &Vec<PrincipalId>) -> Result<(), UpdateSuperUsersError> {
        if has_duplicates(principal_ids) {
            println!(
                "{}do_update_subnet_admins: Error: Duplicate super-users found.",
                LOG_PREFIX
            );
