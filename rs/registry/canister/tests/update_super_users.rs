...
use registry_canister::{
    ...
    mutations::do_update_super_users::{OperationType, UpdateSuperUsersPayload},
};
use std::str::FromStr;

#[test]
fn test_the_anonymous_user_cannot_update_a_subnets_super_users() {
    state_machine_test_on_nns_subnet(|runtime| async move {
        ...
        let initial_super_users = get_value_or_panic::<SubnetRecord>(
            &registry,
        ...
        // The anonymous end-user tries to update a subnet's super users, bypassing
        // the subnet rental canister. This should be rejected.
        let response: Result<(), String> = registry
            .update_("update_super_users", candid, (payload.clone(),))
            .await;
        assert_matches!(response,
                Err(s) if s.contains("is not authorized to call this method: update_super_users"));

        // .. And no change should have happened to the subnet's super users
        let super_users = get_value_or_panic::<SubnetRecord>(
            &registry,
            make_subnet_record_key(subnet_id).as_bytes(),
        )
        .await
        .super_users;
        assert_eq!(super_users, initial_super_users);
...
#[test]
fn test_a_canister_other_than_the_governance_canister_cannot_update_a_subnets_super_users() {
    state_machine_test_on_nns_subnet(|runtime| async move {
        // TEST_ID is used as subnet_id also when creating the initial registry state below.
        let subnet_id = SubnetId::from(PrincipalId::new_subnet_test_id(TEST_ID));
        ...
        // The attacker canister tries to update a subnet's super users, bypassing
...
#[test]
fn test_super_users_cannot_be_updated_for_system_subnet() {
    state_machine_test_on_nns_subnet(|runtime| async move {
        ...
        // Send the update_super_users call via the subnet rental canister...
        ...
        let new_super_user = user_test_id(100).get();
        let payload = UpdateSuperUsersPayload {
            subnet_id,
            operation_type: Some(OperationType::Add(vec![new_super_user])),
        };
        assert!(
            !forward_call_via_universal_canister(
...
#[test]
fn test_super_users_cannot_be_updated_for_non_rented_subnet() {
    state_machine_test_on_nns_subnet(|runtime| async move {
        let subnet_id = SubnetId::from(
            PrincipalId::from_str(
                "bn3el-jdvcs-a3syn-gyqwo-umlu3-avgud-vq6yl-hunln-3jejb-226vq-mae",
...
#[test]
fn test_subnet_rental_canister_can_update_a_subnets_super_users() {
    state_machine_test_on_nns_subnet(|runtime| async move {
        let subnet_id = SubnetId::from(
            PrincipalId::from_str(
                "bn3el-jdvcs-a3syn-gyqwo-umlu3-avgud-vq6yl-hunln-3jejb-226vq-mae",
        ...
        // .. And the new super user should have been added.
        let super_users = get_value_or_panic::<SubnetRecord>(
            &registry,
            make_subnet_record_key(subnet_id).as_bytes(),
        )
        .await
        .super_users;
        assert_eq!(super_users, vec![PrincipalIdPb::from(new_super_user)]);
